...
local Clone = Replicated_Storage.EvolutionLines[evoLine.Value]["3"]:Clone()
local PrimaryPart = Clone.UpperTorso -- the clones primary part
local Offset =  CFrame.new(0, -0.1, Clone:GetExtentsSize().Z * 0.5 + 3.5)
...
game.ReplicatedStorage.RoundStatus.inRound.Changed:Connect(function()
	if game.ReplicatedStorage.RoundStatus.inRound.Value == true then
		Clone:Destroy()
		Clone = Replicated_Storage.EvolutionLines[evoLine.Value]["3"]:Clone()
		PrimaryPart = Clone.UpperTorso -- the clones primary part
		Offset =  CFrame.new(0, -0.1, Clone:GetExtentsSize().Z * 0.5 + 3.5)
		Clone.Parent = ViewportFrame
