repeat wait() until script.Parent.trail.Value ~= nil
...
local function equip(btn)
	...
	if script.Parent.trail.Value == "White Dash" then
...
if script.Parent.trail.Value == statnumbers.lastTrail.Value then
...
script.Parent.MouseButton1Click:Connect(function()
	...
	elseif script.Parent.Status.Value ~= "Locked" and playerFolder.Trails:FindFirstChild(script.Parent.trail.Value) then
		remote:FireServer("Trail", script.Parent.trail.Value)
		SoundService:PlayLocalSound(select)
	end
end)

statnumbers.lastTrail.Changed:Connect(function()
	if script.Parent.trail.Value == statnumbers.lastTrail.Value then
		unlockButton(script.Parent)
		equip(script.Parent)
	elseif script.Parent.Status.Value ~= "Locked" and script.Parent.Status.Value ~= "Buy" then
...
playerFolder.Trails.ChildAdded:Connect(function(item)
	repeat wait() until item.Name ~= "Folder"
	if item.Name == script.Parent.trail.Value then
		if item.Name ~= statnumbers.lastTrail.Value then
